...
package {
	...
	var cont_line:int;
	var img:String;
	var bmpd: BitmapData = new BitmapData(600, 600, false, 0xFFFFFF);
  ...
  trace(e);
		...
		function start(){//ここからメイン					
			...
			return function mapchange(event:Event):void {
				...
				//removeChild(sp);
						sp.x = (stage.stageWidth - 920 +((lng - Number(bmdswlng))*(600/clng)));
						sp.y = (stage.stageHeight - 620 +600- ((lat - Number(bmdswlat))*(600/clat)));//*(600/500)); 
					//trace(mapmarker.x);
					//addChild(sp);
			}
			}
			...
			function onComplete(event:Event):void{
				...
				var pwe:int = ((subbmdnelng[0] - Number(bmdswlng))*(600/clng)-(subbmdswlng[0] - Number(bmdswlng))*(600/clng));
    			var pns:int = ((subbmdnelat[0] - Number(bmdswlat))*(600/clat)-(subbmdswlat[0] - Number(bmdswlat))*(600/clat));
					subbmpd = new BitmapData(pwe, pns, false, 0xFFFFFF);
					subbmp = new Bitmap(subbmpd);
    			
    			var matrix1:Matrix = new Matrix();
				var matrix2:Matrix = new Matrix();
					matrix1.scale(600 /pw, 600/ph);
					...
					subbmp.x = (stage.stageWidth - 920 +((subbmdswlng[0] - Number(bmdswlng))*(600/clng)));
					subbmp.y = (stage.stageHeight - 620 +600- ((subbmdnelat[0] - Number(bmdswlat))*(600/clat)));//*(600/500)); 
					addChild(subbmp);
